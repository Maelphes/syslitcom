...
using the [Boolean operator](#sec-boolean) `AND` as shown below in 
@tbl-test-searchstrategy -- if the search results for lines 4 and 5 are the same, 
all the seed papers were retrieved by the search strategy.
:::

::: {#tbl-test-searchstrategy}
```python
...
5 3 and 4
```
Testing the search strategy using seed papers.
:::
