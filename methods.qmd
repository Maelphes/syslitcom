...
characters with a so-called *wildcard character*, such as `*`, `$`, `?`, `#`.
...
characters or to replace just one character.
...
The kinds of truncation that are available depend on the [syntax](#sec-syntax) 
of the database or search interface. Not all kinds of truncation are generally 
supported.
...
rarely used meaningfully, for instance in valiated [search filters](#sec-filters), such as the search term `diagnostic*[MeSH:noexp]` in the 
