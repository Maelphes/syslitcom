...
known relevant articles, called [*seed papers*](#sec-seedpaper) (also known as 
*core papers*) and cited or citing references. This technique is called 
**citation searching**, **citation analysis** or **citation tracking** 
(@Hirt2024, @Belter2016, @Hinde2015).

The [TARCiS Statement](#sec-tarcis) provides a guideline for citation searching.
(@Hirt2024)

The software [citationchaser](#sec-citationchaser) provides a convenient tool 
for citation searching.(@Haddaway2022b)
...
![co-cited citation searching retrieves publications which share citations with the seed paper](./fig/co-citing.svg){#fig-cocited width=80%}
...
![co-citing citation searching retrieves publications which share references with the seed paper](./fig/co-cited.svg){#fig-cociting width=80%}
...
of the research question as shown in the example below. 


::: {.callout-tip title="Example of an applied filter" collapse="true"}
```python
hypertension/
...
A short qualitative research filter was added in lines 8 to 11, which is 
combined with the other parts of the search strategy in line 12.

:::

::: {.callout-note title="Sources of filter strategies" collapse="true"}
...
## grey literature {#sec-greyliterature}
**Grey literature** (also *gray literature*) is so-called *non-conventional* or 
*informal literature* which "cannot readily be obtained through normal 
bookselling channels", for example conference proceedings, reports, specifications,
supplementary publications, technical notes, theses, translations.(@Wood1982, @Auger2017)

According to @Paez2017 the search for grey literature can be an important 
resource for [systematic literature searching](#sec-syslitsearch).

:::{.callout-tip title="Sources for grey literature searching" collapse="true"}
| URL                                                                  | Description                                           |
|----------------------------------------------------------------------|-------------------------------------------------------|
| <https://www.proquest.com/>                                          | Dissertations and theses (global)                     |
| <https://search.ndltd.org/>                                          | Dissertations and theses (global)                     |
| <https://search.worldcat.org/>                                       | Dissertations and theses (global)                     |
| <https://oatd.org/>                                                  | Dissertations and theses (global)                     |
...
| [doi:10.17026/dans-xtf-47w5](https://doi.org/10.17026/dans-xtf-47w5) | Archive of OpenGrey.eu                                |
: {.striped}
:::
...
Index terms should not be confused with free text terms (DE: *Stichwörter*) 
or [author keywords](#sec-authorkeywords). 
...
the search terms and operators processed. 
...
multiplication and addition. (@ORegan2012)

::: {.callout-info title="Properties of the Boolean Operators"}
...
### proximity operators {#sec-proximityops}
Many databases allow to search for terms depending on the word distance between
each other. The so-called **proximity searching** search uses a 
**proximity operator** in which the allowed distance of the two expressions is 
defined by a number *N*, such as in **adj*N*** or **NEAR/*N***.

For example, the Ovid-syntax search query `((virtual or computer or learning or education) adj3 (simulation or dispensing)).ti,ab`
can retrieve records with phrases such as "virtual dispensing" or "computer based simulation" or "simulation-based education" within the title or abstract. 


::: {.callout-tip title="Syntax examples"}
|                      |                                 |
|----------------------|---------------------------------|
...
| **EBSCO*host***      | `TI (heart N2 transplant)`     |
| **Web of Science**   | `TI=(heart NEAR/2 transplant)` |
| **Scopus**           | `TITLE(heart W/2 transplant)`  |
: Examples of proximity searching in different resources {.striped}
:::



:::{.callout-warning title="Please note"}
- The number *N* sometimes defines the amount of words in between te 
expressions, sometimes it marks the position of the second expression. That is 
why *N* is not always the same for every database or interface. 

- In the case of PubMed, truncation is not possible at the same time as 
proximity searching. In this case, various expressions are necessary to account
for *transplant* as well as *transplantation* or *transplanting*.

- Carefully consider the distance between the expressions. A distance of two 
words between the search terms (as shown above) is often reasonable, as it covers 
frequent expressions such as "transplantation *of the* heart". However, 
sometimes it is necessary to include more adjectives in between.
:::
...
[syntax](#sec-syntax) and [index terms](#sec-indexterm) of the searched 
database. As a consequence, the search strategy needs to be translated for the 
use in other databases.

:::{.callout-tip title="Translation of search strategies"}
There are two main challenges in translating search strategies from one database
or search interface to another:

1. **Different [syntax](#sec-sytax)**.
    The differences in operators, [phrases](#sec-phrases), 
    [nesting](#sec-nesting) or [field codes](#sec-fieldcode) can be managed 
    either by experience, by consulting the resources' own knowledge databases 
    or by using tools such as the 
    [Polyglot Translator](https://sr-accelerator.com/#/polyglot).
    In some cases, there is no objectively clear translation, for instance due 
    to the lack of corresponding syntax. In these cases, the translation is
    a close approximation. 
    

2. **Different [index terms](#sec-indexterms)**.
    In contrast to the syntax, which is fairly similar for most of the 
    databases, the translation of index terms can be very difficult. 
    Some databases do not index records, such as the [Web of Science Core Collection](https://support.clarivate.com/ScientificandAcademicResearch/s/article/Web-of-Science-Core-Collection-No-controlled-vocabulary-or-thesaurus-in-assigning-subject-terms?language=en_US). 
    Others do assign index terms, but don't possess a controlled vocabulary of 
    their own, such as [Scopus](https://service.elsevier.com/app/answers/detail/a_id/21730/).
    Some thesauri are more detailed than others or the hierarchy is structured
    in a different way, so that a 1:1 translation could bear unexpected results.
:::
...
::: {.callout-tip title="Single vs. multiline search strategies"}
...
```python
exp analgesics/
(analge#ic? or anodyne? or antinoceptive?).ti,ab,kw.
*pain management/
(1 or 2) and 3
```
:::

::: {.callout-tip title="Testing your search strategy"}
You can use [seed papers](#sec-seedpaper) to test whether your search strategy 
picks them up. In the first step you search for the seed papers using their
unique identifiers such as [PMID or PMCID](#sec-pmid-pmcid) or [DOI](#sec-doi).
Second, you connect the found seed papers with the rest of your search strategy 
using the [Boolean operator](#sec-boolean) `AND` as shown below -- if the search 
results for lines 4 and 5 are the same, 
all the seed papers were retrieved by the search strategy.
 
```python
hypertension/ or antihypertensive agents/
deprescriptions/
1 and 2
("39209778" or "39094592" or "28437544" or "26591140" or "29678989").ui.
3 and 4
```
...
interested in reports of adverse effects. (@Golder2006a)
In order to do that the subheading can be searched alone as a so-called 
**floating subheading**, for example `adverse effects[sh]` in PubMed or 
`ae.fs` in Ovid MEDLINE.
...
@Munn2018, @Munn2018a or @Pearson2015)
