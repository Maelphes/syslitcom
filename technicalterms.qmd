...
e.g. journal articles, theses or conference papers. 
A study by @Shiely2024 found that the abstract is one of the most important parts
of a publication, as it is usually the first part and sometimes the only part of
the paper which is read. It is usually the only part of the full text that is 
freely available, it is easy to read and can be retrieved from bibliographic 
databases. Title, abstract are the key elements of a textword search. 
(@Cals2013, @Pitkin1998)



An **structured abstract** (as shown below in @fig-abstract) features labeled 
subsections, similar to the [IMRaD format](#sec-imrad) used for structuring 
scientific articles. A main purpose of such a structure is to make the abstract 
more informative to the reader.


![Structured Abstract for the PRISMA-S Extension by @Rethlefsen2021](./fig/abstract.png){#fig-abstract width=100%}


## ambiguity {#sec-ambiguity}
A term or statement which has more than one possible meaning or definition is 
ambiguous and can exist in very different contexts. 

Ambiguity is often caused by *polysemy*, i.e. the fact that one 
expression can have more than one meaning, or by *homonymy*, in which independent 
words possess identical spelling, but bear different meanings.

Ambiguity proves to be an obstacle in creating search strategies, 
...
5. `discharge` could mean the releasing an inpatient from a hospital or in 
another context the releasing of an electric charge.
6. `lead` can be the verb *to lead*, which can mean to direct something or 
someone with authority, or the noun *lead*, which can mean various things, such 
as a heavy metal element (Pb) or the thin graphite cylinder within pencils 
("pencil lead").
7. `dialysis` is a chemical technique of separating molecules in solution. It is 
also the colloquial term for *renal dialysis*, a medical treatment using the 
aforementioned technique to filter blood and thus temporarily replace the 
kidney function.
:::
...
describing the content of the publication. These keywords are usually part of or 
located close to the [abstract](#sec-abstract), as shown in @fig-abstract. 

Author keywords don't necessarily correspond to [controlled vocabulary](#sec-thesaurus) 
(i.e. index terms, which are assigned to the records by the database) and should not be confused 
with them. (@Neveol2010)

::: {.callout-tip title="Examples"}
Within PubMed the [data field](#sec-datafield) `[Other Term]`) 
can be searched for author keywords. The [PubMed Record for the PRISMA-S Extension](https://pubmed.ncbi.nlm.nih.gov/33499930/?format=pubmed) by @Rethlefsen2021 features the following Keywords in PubMed (as seen in as @fig-abstract):
...
They are usually searched as part of the free text search. (DE: *Stichwortsuche*)

In PubMed, the `OT` field containing the author keywords is automatically searched 
...
In [Ovid MEDLINE](https://ospguides.ovid.com/OSPguides/medline.htm#KF), the corresponding field codes `.kw` or `.kf` can be used for 
...
### NNR {#sec-nnr}
The **Number Needed to Read (NNR)** is another common metric, defined as the 
number of records necessary to screen to find a relevant record. The NNR is 
simply the inverse of the precision:
...
**Comma-separated values (CSV)** is a plain text data format to store tabular information, such as bibliographic records. CSV is described in detail in [RFC 4180](https://www.rfc-editor.org/rfc/rfc4180). 

A CSV file consists of multiple lines of comma-separated [data fields](#sec-datafield). The first line can be used as a header line listing the names of all columns. Otherwise, each line in the file contains one record with its field values separated by commas. To be interpreted correctly the fields must maintain the same order over all lines and empty fields must not be omitted.
...
Data fields PMID, AU, TI shown for three records
:::
...
@Greenhalgh2025 defines evidence-based medicine as "the use of mathematical 
estimates of the risk of benefit and harm, derived from high-quality research 
on population samples, to inform clinical decision-making in the diagnosis, 
...
In some cases, an article is published in more than one place at once. 
There are full-text databases, such as [PubMed Central](#sec-pmc), which provide 
access to a full text article parallel to the publisher. For example, the 
guideline [PRISMA-S](#sec-prisma) by @Rethlefsen2021 is available via 
[the journal website](https://doi.org/10.1186/s13643-020-01542-z) or as a 
[PubMed Central record](https://pmc.ncbi.nlm.nih.gov/articles/PMC7839230/).
...
In case you are affiliated with a university or similar institution, it is 
always a good idea to consult your institution's library catalog for all 
options available to you for accessing a certain publication.
:::


## indexing {#sec-indexing}

**Subject indexing** or just **indexing** (DE: Beschlagwortung, Verschlagwortung) 
is the process in which [index terms](#sec-indexterms) are assigned to 
[records](#sec-dataset). This is done by the library or database provider in 
order to indicate what the referenced document or medium is about, independent 
of its explicit title or abstract. In other words, an indexed record can be 
retrieved systematically based on its contextual meaning and implicit contents, 
rather than by searching verbatim expressions in the text.
...
| [nano](https://nano-editor.org/) 	            | Windows, Linux, macOS              	|
...
3. Testing [search strategies](#sec-searchstrategy).
:::
...
Examples for syntax in different search interfaces
:::
...
Commonly known databases and their thesauri
:::
...
## vocabulary mismatch {#sec-vocabularymismatch}

Vocabulary mismatch occurs when an object or circumstance is called or described
by different individuals using different expressions, which means the vocabularies 
of these individuals do not match. This is a well-known phenomenon in 
information retrieval. (@Furnas1987) 

For instance, researchers attempt to retrieve literature using search terms 
which seem relevant to them or are commonly used in their field, whereas the 
authors (perhaps from a different special field) might use different (or 
sometimes unspecific or uncommon) expressions to describe the same
subject.

::: {.callout-tip title="Example"}
A research group is interested in hypertension, which is why they use the 
search term `hypertension` in their free text search. However, some of the 
studies relevant to their research question mention
...
without the occurrence of the word *hypertension* in title or abstract. 
Without additional search terms these publications are not retrieved in the search.

::: 

In order to mitigate this problem systematic literature searches usually 
employ synonyms, [truncation](#sec-truncation), [index terms](#sec-indexterms), 
and [proximity operators](#sec-proximityops), or are supplemented by non-Boolean
techniques, such as [citation searching](#sec-citationsearching).
...
Commonly used wildcard symbols
:::


:::{#tbl-wildcards2 .striped}
...
Wildcard symbols and the number of characters *n* they are allowed to replace in different search interfaces
:::
